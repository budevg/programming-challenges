...
runState' ::
  ...
  -> s
  -> (a, s)
runState' a = \s -> let ExactlyOne (a', s') = runStateT a s
                    in (a', s')
...
exec' ::
  ...
  -> s
  -> s
exec' a = \s -> let ExactlyOne (_, s') = runStateT a s
                in s'
...
eval' ::
  ...
  -> s
  -> a
eval' a = \s -> let ExactlyOne (a', _) = runStateT a s
                in a'
...
distinct' ::
  ...
  List a
  -> List a
distinct' as = eval' scan S.empty
  where scan = filtering isDistinct as
        isDistinct x = state' $ \s ->
          if S.member x s
          then (False, s)
...
distinctF ::
  ...
  List a
  -> Optional (List a)
distinctF as = evalT scan S.empty
  where scan = filtering check as
        check x
...
instance Applicative f => Applicative (OptionalT f) where
  ...
  (<*>) f a = OptionalT $
              let f' = runOptionalT f
                  a' = runOptionalT a
              in (applyOptional <$> f') <*> a'
