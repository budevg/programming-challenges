...
toList ::
  ListZipper a
  -> List a
toList (ListZipper Nil a rs) = a :. rs
toList (ListZipper (l :. ls) a rs) =
  toList $ ListZipper ls l ( a :. rs)
...
setFocus ::
  ...
  -> ListZipper a
  -> ListZipper a
setFocus a (ListZipper ls _ rs) = ListZipper ls a rs
...
findLeft p (ListZipper (l :. ls) a rs)
  | p l = IsZ $ ListZipper ls l (a :. rs)
  | otherwise = findLeft p (ListZipper ls l (a :. rs))
...
findRight p (ListZipper ls a (r :.rs))
  | p r = IsZ $ ListZipper (a :. ls) r rs
  | otherwise = findRight p (ListZipper (a :. ls) r rs)
...
moveLeftLoop (ListZipper Nil a rs) =
  let (a' :. ls) = reverse (a :. rs)
  in ListZipper ls a' Nil
...
moveRightLoop (ListZipper ls a Nil) =
  let (a' :. rs) = reverse (a :. ls)
  in ListZipper Nil a' rs
...
moveLeft (ListZipper (l :. ls) a rs) = IsZ $
  ListZipper ls l (a :. rs)
...
moveRight (ListZipper ls a (r :. rs)) = IsZ $
  ListZipper (a :. ls) r rs
...
swapLeft (ListZipper (l :. ls) a rs ) = IsZ $
  (ListZipper (a :. ls) l rs )
...
swapRight (ListZipper ls a (r :. rs)) = IsZ $
  (ListZipper ls r (a :. rs))
...
moveLeftN n z
  | n < 0 = moveRightN (-n) z
  | n == 0 = IsZ z
  | otherwise = case moveLeftN (n - 1) z of
                     IsNotZ -> IsNotZ
                     IsZ z' -> moveLeft z'
...
moveRightN n z
  | n < 0 = moveLeftN (-n) z
  | n == 0 = IsZ z
  | otherwise = case moveRightN (n - 1) z of
                     IsNotZ -> IsNotZ
                     IsZ z' -> moveRight z'
...
nth n z@(ListZipper ls _ rs)
  | n <= lenLs = moveLeftN (lenLs -n) z
  | n - lenLs - 1 <= lenRs =  moveRightN (lenRs - (n - lenLs - 1)) z
  | otherwise = IsNotZ
  where lenLs = length ls
        lenRs = length rs
...
-- prop> rights (end lz) == Nil
  ...
  ListZipper a
  -> ListZipper a
end (ListZipper ls a rs) =
  let (a' :. ls') = (reverse rs) ++ (a :. ls)
  in ListZipper ls' a' Nil
...
start ::
  ListZipper a
  -> ListZipper a
start (ListZipper ls a rs) = let (a' :. rs') = (reverse ls) ++ (a :. rs)
                             in ListZipper Nil a' rs'
...
deletePullLeft (ListZipper (l :. ls) _ rs) = IsZ $
  (ListZipper ls l rs)
...
deletePullRight (ListZipper ls _ (r :. rs)) = IsZ $
  (ListZipper ls r rs)
...
insertPushLeft ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushLeft a' (ListZipper ls a rs) =
  ListZipper (a :. ls) a' rs
...
insertPushRight ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushRight a' (ListZipper ls a rs) =
  ListZipper ls a' (a :. rs)
