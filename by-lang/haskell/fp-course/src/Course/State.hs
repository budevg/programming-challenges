...
firstRepeat ::
  ...
  List a
  -> Optional a
firstRepeat as = eval scan S.empty
  where
    scan = findM isRepeat as
    isRepeat x = State $ \s ->
      if S.member x s
      then (True, s)
...
isHappy ::
  Integer
  -> Bool
isHappy num = case firstRepeat (produce sumOfSquareDigits num') of
              Full 1 -> True
              _ -> False
  where num' = fromInteger num
        sumOfSquareDigits n = sum $ map (\x -> x * x) (digits n)
