...
instance Functor ListZipper where
  (<$>) f (ListZipper ls a rs) =
    ListZipper (f <$> ls) (f a) (f <$> rs)
...
toList ::
  ListZipper a
  -> List a
toList (ListZipper ls a rs) = (reverse ls) ++ (a :. rs)
...
findLeft p (ListZipper ls a rs) =
  let (rs', ls') = break p ls
  in case ls' of
    Nil -> IsNotZ
    (a' :. ls'') -> IsZ (ListZipper ls'' a' ((reverse rs') ++ (a :. rs)))
...
findRight p (ListZipper ls a rs) =
  let (ls', rs') = break p rs
  in case rs' of
    Nil -> IsNotZ
    (a' :. rs'') -> IsZ (ListZipper ((reverse ls') ++ (a :. ls)) a' rs'')
...
moveLeftLoop (ListZipper ls a rs) =
  case ls of
    (a' :. ls') -> ListZipper ls' a' (a :. rs)
    Nil -> let (a'' :. ls'') = (reverse rs) ++ a :. Nil
...
moveRightLoop (ListZipper ls a rs) =
  case rs of
    (a' :. rs') -> ListZipper (a :. ls) a' rs'
    Nil -> let (a'' :. rs'') = (reverse ls) ++ a :. Nil
...
moveLeft ::
  ListZipper a
  -> MaybeListZipper a
moveLeft (ListZipper ls a rs) =
  case ls of
    Nil -> IsNotZ
    (a' :. ls') -> IsZ $ ListZipper ls' a' (a :. rs)
...
moveRight ::
  ListZipper a
  -> MaybeListZipper a
moveRight (ListZipper ls a rs) =
  case rs of
    Nil -> IsNotZ
    (a' :. rs') -> IsZ $ ListZipper (a :. ls) a' rs'
...
swapLeft ::
  ListZipper a
  -> MaybeListZipper a
swapLeft (ListZipper ls a rs) =
  case ls of
    Nil -> IsNotZ
    (l :. ls') -> IsZ $ ListZipper (a :. ls') l rs
...
swapRight ::
  ListZipper a
  -> MaybeListZipper a
swapRight (ListZipper ls a rs) =
  case rs of
    Nil -> IsNotZ
    (r :. rs') -> IsZ $ ListZipper ls r (a :. rs')
...
nth ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
nth n z = let IsZ z' = fromList $ (toList z)
          in moveRightN n z'
...
-- prop> rights (end lz) == Nil
  ...
  ListZipper a
  -> ListZipper a
end z = let IsZ z' = moveRightN (length $ rights z) z
        in z'
...
start ::
  ListZipper a
  -> ListZipper a
start z = let IsZ z' = moveLeftN (length $ lefts z) z
          in z'
...
insertPushLeft ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushLeft a' (ListZipper ls a rs) = ListZipper (a :. ls) a' rs
...
insertPushRight ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushRight a' (ListZipper ls a rs) = ListZipper ls a' (a :. rs)
...
-- /Tip:/ Use `zipWith`
  (<*>) (ListZipper fls f frs) (ListZipper als a ars) =
    ListZipper (zipWith ($) fls als) (f a) (zipWith ($) frs ars)
...
instance Extend ListZipper where
  (<<=) f z = ListZipper ls' (f z) rs'
    where ls' = unfoldr (helper moveLeft) z
          rs' = unfoldr (helper moveRight) z
          helper move z' = case move z' of
            IsNotZ -> Empty
            IsZ z'' -> Full (f z'', z'')
...
instance Traversable ListZipper where
  traverse f (ListZipper ls a rs) =
    lift3 ListZipper (traverse f ls) (f a) (traverse f rs)
