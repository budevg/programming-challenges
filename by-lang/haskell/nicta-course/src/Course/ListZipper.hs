...
findLeft p (ListZipper l a r)
  | isEmpty l = IsNotZ
  | otherwise = let (a' :. l') = l
                    z' = ListZipper l' a' (a :. r)
                in if p a' then IsZ z' else findLeft p z'
...
findRight p (ListZipper l a r)
  | isEmpty r = IsNotZ
  | otherwise = let (a' :. r') = r
                    z' = ListZipper (a :. l) a' r'
                in if p a' then IsZ z' else findRight p z'
...
moveLeftLoop (ListZipper l a r) =
  if isEmpty l then let (a' :. l') = reverse (a :. r)
                        in ListZipper l' a' Nil
  else let (a' :. l') = l
       in ListZipper l' a' (a :. r)
...
moveRightLoop (ListZipper l a r) =
  if isEmpty r then let (a' :. r') = reverse (a :. l)
                        in ListZipper Nil a' r'
  else let (a' :. r') = r
       in ListZipper (a :. l) a' r'
...
moveLeft (ListZipper l a r) =
  if isEmpty l then IsNotZ
  else let (a' :. l') = l
       in IsZ $ ListZipper l' a' (a :. r)
...
moveRight (ListZipper l a r) =
  if isEmpty r then IsNotZ
  else let (a' :. r') = r
       in IsZ $ ListZipper (a :. l) a' r'
...
moveLeftN' n z@(ListZipper l _ _)
  ...
  | otherwise = let IsZ z' = moveLeftN n z
                    in Right z'
...
moveRightN' n z@(ListZipper _ _ r)
  ...
  | otherwise = let IsZ z' = moveRightN n z
                    in Right z'
...
nth ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
nth n z@(ListZipper l _ r) =
  helper n (length l) (length r) z
  where helper n' nl' rl' z' =
          if n' < nl' then moveLeftN (nl' - n') z'
          else if n' == nl' then IsZ z'
               else if (n' - nl') < rl' then moveRightN (n' - nl') z'
                    else IsNotZ
...
end (ListZipper l a r) = let (a' :. l') = reverse r
                         in ListZipper (l' ++ (a :. l)) a' Nil
...
start (ListZipper l a r) = let (a' :. r') = reverse l
                           in ListZipper Nil a' (r' ++ (a :. r))
...
-- /Tip:/ Use `zipWith`
  (<*>) (ListZipper la a ra) (ListZipper lb b rb) =
    ListZipper (zipWith apply la lb) (a b) (zipWith apply ra rb)
    where apply f x = f x
