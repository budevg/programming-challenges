...
noneof ::
  Chars
  -> Parser Char
noneof s = satisfy (\c -> c `notElem` s)
...
betweenCharTok ::
  ...
  -> Parser a
  -> Parser a
betweenCharTok c1 c2 pa = between (is c1) (is c2) pa
...
hex ::
  Parser Char
hex = do
  h <- thisMany 4 (satisfy isHexDigit)
  case readHex h of
    Empty -> failed
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
satisfyAll ps = satisfy $ \c -> check ps c
  where check Nil _ = True
        check (p' :. ps') c' = if p' c' then check ps' c' else False
...
satisfyAny ::
  List (Char -> Bool)
  -> Parser Char
satisfyAny ps = satisfy $ \c -> check ps c
  where check Nil _ = False
        check (p' :. ps') c' = if p' c' then True else check ps' c'
