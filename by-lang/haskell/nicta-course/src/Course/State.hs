...
findM pm (x :. xs) = do
  cond <- pm x
  if cond then return (Full x) else findM pm xs
...
isHappy ::
  ...
  -> Bool
isHappy n =
  case firstRepeat sums of
    (Full 1) -> True
    _ -> False
  where sums = 1 :. (produce sumSquareDigits (P.fromIntegral n))
        sumSquareDigits x = sum $ map squareDigit (show' x)
        squareDigit = (\x -> x * x) . digitToInt
