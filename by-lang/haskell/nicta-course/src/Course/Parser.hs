...
bindParser ::
  ...
  -> Parser a
  -> Parser b
bindParser f (P pa) = P $ \i ->
  case pa i of
    ErrorResult e -> ErrorResult e
    Result i' a -> case parse (f a) $ i'  of
      ErrorResult e -> ErrorResult e
...
list ::
  Parser a
  -> Parser (List a)
list pa = h ||| valueParser Nil
  where h = flbindParser pa (\a -> mapParser (a :.) $ list pa)
...
list1 ::
  Parser a
  -> Parser (List a)
list1 pa = flbindParser pa (\a -> mapParser (a :.) $ list pa)
...
satisfy p = P $ \i ->
  case i of
    Nil -> ErrorResult Failed
    (c :. cs) -> if p c then Result cs c else ErrorResult Failed
...
natural ::
  Parser Int
natural = (list1 digit) `flbindParser` (\s -> case read s of
                                           Empty -> failed
...
sequenceParser (pa :. pas) = flbindParser pa (\a -> mapParser (a :.) $ sequenceParser pas)
...
personParser = do
  ...
  _ <-spaces1
  pSureName <- surnameParser
  _ <-spaces1
  pSmoker <- smokerParser
  _ <-spaces1
  pPhone <- phoneParser
  let person = Person pAge pFirstName pSureName pSmoker pPhone
  valueParser $ person
