...
compose ::
  ...
  -> Lens a b
  -> Lens a c
compose (Lens sbc gbc) (Lens sab gab) = Lens sac gac
  where
    sac a c = sab a (sbc (gab a) c)
    gac a = gbc (gab a)
...
setCityAndLocality = set $ personCityL *** localityL
  where personCityL = cityL |. localityL |. addressL
...
setStreetOrState ::
  ...
  -> String
  -> Either Person Locality
setStreetOrState = set $ street ||| state
  where street = streetL |. addressL
        state = stateL
...
modifyCityUppercase ::
  Person
  -> Person
modifyCityUppercase = personCityL %~ (map toUpper)
  where personCityL = cityL |. localityL |. addressL
